...
use crate::{Lookup, fmt_list, writeln};
...
pub fn emit_skiplist(ranges: &[Range<u32>]) -> Lookup {
    let first_code_point = ranges.first().unwrap().start;
    let points = ranges.iter().flat_map(|r| [r.start, r.end]);
    let mut offset = 0;
    ...
    writeln!(file, "use super::ShortOffsetRunHeader;\n");
    writeln!(
        file,
    ...
    bytes_used += 4 * short_offset_runs.len();
    writeln!(file, "static OFFSETS: [u8; {}] = {coded_offsets:?};", coded_offsets.len());
    bytes_used += coded_offsets.len();
    ...
    Lookup { file, bytes_used, desc: "skiplist" }
}
